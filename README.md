...
A production-grade, local Retrieval-Augmented Generation (RAG) chatbot system built for **Jai Hind College (Autonomous), Mumbai**. The system combines web scraping, vector embeddings, local vector search, SQLite session state tracking, and local LLM inferencing via Ollama.
...
* **Local Private Inferencing:** Fully offline-capable LLM processing through Ollama without sending sensitive data to external APIs.
* **Semantic Vector Retrieval:** Uses `sentence-transformers/all-MiniLM-L6-v2` (384 dimensions) and local Qdrant instance for cosine-similarity retrieval.
...
* **Automatic Chat Summarization:** Automatically generates 3-to-4 word conversational titles for saved chat sessions using local LLM inference.
...
├── gradio_app.py          # Interactive web UI built with Gradio Blocks
...
| **Backend API Host** | `http://127.0.0.1:8000` | `server.py`, `gradio_app.py` |
| **Frontend UI Host** | `http://127.0.0.1:7860` | `gradio_app.py` |
| **Ollama Completion Endpoint** | `http://127.0.0.1:11434/v1/chat/completions` | `server.py` |
| **Temperature** | `0.2` | `server.py` |
